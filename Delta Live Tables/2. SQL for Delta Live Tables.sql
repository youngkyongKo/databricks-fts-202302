...
-- MAGIC 
-- MAGIC 지난 수업에서는 이 노트북을 델타 라이브 테이블(DLT) 파이프라인으로 활용하는 과정을 살펴보았습니다. 이제 델타 라이브 테이블에서 사용하는 구문을 더 잘 이해하기 위해 이 노트북의 내용을 살펴보겠습니다.
-- MAGIC 
-- MAGIC 이 Notebook은 SQL을 사용하여 기본적으로 Databricks Workspace 에 로드되는 Databricks 가 제공하는 예제 데이터 세트를 기반으로 간단한 Multi-Hop 아키텍처를 함께 구현하는 델타 라이브 테이블을 선언합니다.
-- MAGIC 
